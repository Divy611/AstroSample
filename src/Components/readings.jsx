...
function FeaturedCard({ reading, index }) {
    ...
    return (
        <motion.article ref={ref} whileHover={{ y: -8, boxShadow: `0 28px 70px ${reading.accentColor}22` }} transition={{ duration: 0.3, ease: 'easeOut' }} style={{
            background: '#ffffff',
            ...
            <motion.div variants={modalVariants} initial="hidden" animate="visible" exit="exit" onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: '780px', background: '#ffffff', border: `1.5px solid ${reading.accentColor}33`, borderRadius: '28px', overflow: 'hidden', boxShadow: `0 40px 100px ${reading.accentColor}18, 0 0 0 1px rgba(167,139,250,0.1)`, marginTop: '4rem' }}>
                ...
                <div style={{ padding: 'clamp(2rem, 5vw, 3.5rem)' }}>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '1.5rem' }}>
                        <motion.button whileHover={{ scale: 1.1, background: 'rgba(167,139,250,0.12)' }} whileTap={{ scale: 0.95 }} onClick={onClose} style={{
                            width: '36px', height: '36px', borderRadius: '50%',
                            background: 'rgba(167,139,250,0.07)',
                            border: '1.5px solid rgba(167,139,250,0.22)',
                            color: '#7c5cbf', fontSize: '1.2rem',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem', padding: '1rem 1.25rem', borderRadius: '14px', background: 'rgba(245,240,255,0.7)', border: '1px solid rgba(167,139,250,0.15)', marginBottom: '2.5rem' }}>
                        ...
                        <div>
                            ...
                            <div style={{ color: '#9585b0', fontSize: '0.75rem' }}>Purple Celestia Practitioner · {reading.date}</div>
                        </div>
                    </div>
                    ...
                    <div style={{ color: '#6b5c8a', fontSize: '1rem', lineHeight: 2, marginBottom: '2rem' }}>
                        ...
                        <div style={{ margin: '2.5rem 0', padding: '1.75rem 2rem', borderLeft: `4px solid ${reading.accentColor}`, background: `${reading.accentColor}08`, borderRadius: '0 16px 16px 0' }}>
                            <p style={{ color: '#2d2438', fontSize: '1.1rem', fontStyle: 'italic', lineHeight: 1.75, fontFamily: "'Cormorant Garamond', serif", fontWeight: 600 }}>"The cosmos does not impose fate — it illuminates tendency. The aware soul uses this light to navigate, not to surrender."</p>
                    ...
                    <div style={{ padding: '1.75rem 2rem', borderRadius: '18px', background: 'linear-gradient(135deg, rgba(245,240,255,0.9), rgba(255,248,255,0.95))', border: '1.5px solid rgba(167,139,250,0.2)', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1.5rem' }}>
                        <div>
                            <div style={{ color: '#2d2438', fontWeight: 700, fontSize: '1rem', marginBottom: '0.3rem' }}>Curious about your own chart?</div>
                            <div style={{ color: '#9585b0', fontSize: '0.83rem' }}>Book a personalised {reading.category} reading today.</div>
                        </div>
                        <motion.button whileHover={{ scale: 1.04, boxShadow: `0 8px 28px ${reading.accentColor}33` }} whileTap={{ scale: 0.97 }} style={{
                            padding: '0.75rem 1.75rem',
                            background: `linear-gradient(135deg, ${reading.accentColor}, ${reading.accentColor}bb)`,
                            border: 'none', borderRadius: '50px',
                            color: '#fff', fontSize: '0.88rem', fontWeight: 700,
                            cursor: 'pointer', letterSpacing: '0.04em', whiteSpace: 'nowrap',
                        }}>Book a Reading →</motion.button>
                    </div>
                </div>
            </motion.div>
        </motion.div>
    ...
    return (
        <div ref={ref} style={{ background: 'linear-gradient(135deg, rgba(245,240,255,0.95), rgba(255,248,255,0.98))', border: '1.5px solid rgba(167,139,250,0.22)', borderRadius: '24px', padding: 'clamp(2rem, 4vw, 3rem)', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '2rem', position: 'relative', overflow: 'hidden', boxShadow: '0 8px 40px rgba(124,92,191,0.07)' }}>
            ...
            <div style={{ maxWidth: '420px' }}>
                <div style={{ color: '#7c5cbf', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '0.5rem' }}>✦ Cosmic Dispatch</div>
                <h3 style={{ color: '#2d2438', fontSize: 'clamp(1.2rem, 2.5vw, 1.6rem)', fontWeight: 800, fontFamily: "'Cormorant Garamond', serif", lineHeight: 1.3, marginBottom: '0.5rem' }}>Receive Weekly Insights</h3>
                <p style={{ color: '#6b5c8a', fontSize: '0.85rem', lineHeight: 1.7 }}>Planetary transits, lunar forecasts, and new readings — delivered to your inbox every Sunday.</p>
            </div>
                ...
                {submitted
                    ...
                    : <motion.form key="form" onSubmit={handleSubmit} style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap' }}>
                        <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" style={{
                            ...
                            color: '#2d2438', fontSize: '0.875rem', outline: 'none',
                            minWidth: '220px', transition: 'border-color 0.2s',
                        }} onFocus={(e) => (e.target.style.borderColor = 'rgba(124,92,191,0.55)')} onBlur={(e) => (e.target.style.borderColor = 'rgba(167,139,250,0.25)')} />
                        <motion.button whileHover={{ scale: 1.03, boxShadow: '0 8px 24px rgba(124,92,191,0.3)' }} whileTap={{ scale: 0.97 }} type="submit" style={{
        ...
        <div style={{ background: '#faf9f7', minHeight: '100vh', color: '#2d2438', fontFamily: "'Inter', sans-serif", overflowX: 'hidden' }}>
            <section ref={heroRef} style={{
                ...
                <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: '9rem 1.5rem 4rem', maxWidth: '760px', margin: '0 auto' }}>
                    ...
                    }}>✦ The Purple Celestia Compendium</motion.div>
                    <h1 ref={titleRef} style={{
                        fontSize: 'clamp(2.4rem, 6vw, 4.5rem)', fontWeight: 800,
                        lineHeight: 1.1, letterSpacing: '-0.02em',
                        fontFamily: "'Cormorant Garamond', serif", marginBottom: '1.25rem',
                    }}>
                        <span style={{ background: 'linear-gradient(135deg, #3d2b6b 30%, #7c5cbf 70%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Wisdom Written</span>
                        <br />
                        <span style={{ background: 'linear-gradient(135deg, #a78bfa, #c4b5fd, #f0abfc)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>in the Stars</span>
                    </h1>
                    <p ref={subtitleRef} style={{
                        fontSize: 'clamp(0.95rem, 2vw, 1.12rem)', lineHeight: 1.85,
                        color: '#6b5c8a', maxWidth: '540px',
            ...
            <section ref={gridRef} style={{
                ...
                <div className="reveal-up" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', marginBottom: '3rem', paddingTop: '2rem' }}>
                    <div>
                        ...
                        <p style={{ color: '#9585b0', fontSize: '0.82rem', marginTop: '0.25rem' }}>{filteredReadings.length} article{filteredReadings.length !== 1 ? 's' : ''} found</p>
                    </div>
