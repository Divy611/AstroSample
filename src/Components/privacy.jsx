    ...
    return (
        <div style={{ background: '#faf9f7', minHeight: '100vh', color: '#2d2438', fontFamily: "'Inter', sans-serif", overflowX: 'hidden' }}>
            <section style={{ position: 'relative', padding: '10rem 1.5rem 5rem', textAlign: 'center', overflow: 'hidden', background: 'linear-gradient(160deg, #fdf8ff 0%, #f5f0ff 45%, #fff8f5 100%)' }}>
                ...
                <h1 ref={titleRef} style={{ fontSize: 'clamp(2rem, 5vw, 3.8rem)', fontWeight: 800, fontFamily: "'Cormorant Garamond', serif", lineHeight: 1.1, letterSpacing: '-0.02em', marginBottom: '1rem', background: 'linear-gradient(135deg, #3d2b6b 30%, #7c5cbf)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Privacy Policy</h1>
                <p ref={subtitleRef} style={{ color: '#6b5c8a', fontSize: '0.9rem', maxWidth: '480px', margin: '0 auto 1.5rem', lineHeight: 1.8 }}>Your trust is sacred to us. This policy explains what we collect, why we collect it, and how we protect it.</p>
            ...
            <section style={{ maxWidth: '820px', margin: '0 auto', padding: '0 1.5rem 8rem' }}>
                ...
                {SECTIONS.map((section, i) => (
                    <div key={section.id} id={section.id}>
                        ...
                        {i < SECTIONS.length - 1 && <div style={{ height: '1px', margin: '0 0 3rem', background: 'linear-gradient(90deg, transparent, rgba(167,139,250,0.2), transparent)' }} />}
                    </div>
                ))}
                <div style={{ marginTop: '2rem', padding: '2rem', background: 'linear-gradient(135deg, rgba(245,240,255,0.9), rgba(255,248,255,0.95))', border: '1.5px solid rgba(167,139,250,0.2)', borderRadius: '20px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1.5rem', boxShadow: '0 4px 28px rgba(124,92,191,0.07)' }}>
                    <div>
                        <div style={{ color: '#2d2438', fontWeight: 700, fontSize: '1rem', marginBottom: '0.3rem' }}>Questions about this policy?</div>
