        ...
        <section ref={ref} style={{ padding: '7rem 1.5rem', background: 'linear-gradient(180deg, #fdf8ff 0%, rgba(245,240,255,0.6) 60%, #faf9f7 100%)', position: 'relative', overflow: 'hidden' }}>
            {[...Array(14)].map((_, i) => (
                <div key={i} style={{
                    ...
                    borderRadius: '50%',
                    background: i % 2 === 0 ? 'rgba(167,139,250,0.55)' : 'rgba(104,150,200,0.45)',
                    top: `${8 + (i * 19 % 84)}%`,
                    left: `${4 + (i * 27 % 92)}%`,
                    animation: `twinkleC ${2 + (i % 3)}s ease-in-out infinite`,
            ...
            <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
                <div className="path-title" style={{ textAlign: 'center', marginBottom: '4.5rem' }}>
                    ...
                    <h2 style={{ fontSize: 'clamp(1.75rem, 4vw, 3rem)', fontWeight: 800, fontFamily: "'Cormorant Garamond', serif", background: 'linear-gradient(135deg, #1e3a5f, #7c5cbf)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: '0.75rem' }}>
                        What Happens After You Write
                    </h2>
                    <p style={{ color: '#4a6080', fontSize: '0.97rem', maxWidth: '400px', margin: '0 auto', lineHeight: 1.8 }}>
                        Every conversation follows a clear, personal path — no confusion, no guesswork.
                    </p>
                </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '2rem', position: 'relative', zIndex: 1 }}>
                        {steps.map((s, i) => (
                            <motion.div
                                key={s.title}
                                className="path-step"
                                whileHover={{ y: -8, boxShadow: `0 20px 50px ${s.color}1a` }}
                                transition={{ duration: 0.28, ease: 'easeOut' }}
                                style={{ background: '#ffffff', border: '1.5px solid rgba(167,139,250,0.16)', borderRadius: '22px', padding: '2rem 1.5rem', textAlign: 'center', boxShadow: '0 4px 24px rgba(124,92,191,0.05)', willChange: 'transform', position: 'relative' }}
                            >
                                <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '1.25rem' }}>
                                    <div className="path-node" style={{
                                        width: '56px', height: '56px', borderRadius: '50%',
                                        background: `radial-gradient(ellipse, ${s.color}20 0%, ${s.color}08 70%)`,
                                        border: `1.5px solid ${s.color}38`,
        ...
        <section ref={ref} style={{ background: 'linear-gradient(135deg, #1a2d4a, #2d1f55)', overflow: 'hidden', position: 'relative' }}>
            <div style={{ borderBottom: '1px solid rgba(167,139,250,0.18)', padding: '0.9rem 0', overflow: 'hidden' }}>
                <div className="trust-ticker" style={{ display: 'flex', gap: '3rem', whiteSpace: 'nowrap', width: 'max-content' }}>
                    {ticker.map((t, i) => (
                        <span key={i} style={{ color: i % 3 === 0 ? 'rgba(167,139,250,0.7)' : i % 3 === 1 ? 'rgba(104,150,200,0.6)' : 'rgba(255,255,255,0.25)', fontSize: '0.82rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase' }}>{t}</span>
                    ))}
                </div>
            </div>
            ...
            <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '5rem 1.5rem' }}>
                <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
                    ...
                    <h3 style={{ color: '#f0eeff', fontSize: 'clamp(1.5rem, 3vw, 2.25rem)', fontWeight: 800, fontFamily: "'Cormorant Garamond', serif", lineHeight: 1.25 }}>
                        This is a safe space for honest conversations.
                    </h3>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.25rem' }}>
                    {trustPoints.map((t) => (
                        <motion.div
                            ...
                            whileHover={{ borderColor: 'rgba(167,139,250,0.35)', background: 'rgba(167,139,250,0.1)' }}
                            transition={{ duration: 0.22 }}
                            style={{ padding: '1.75rem', borderRadius: '18px', background: 'rgba(167,139,250,0.06)', border: '1px solid rgba(167,139,250,0.16)', transition: 'background 0.22s, border-color 0.22s' }}
                        >
            ...
            <div style={{ borderTop: '1px solid rgba(167,139,250,0.18)', padding: '0.9rem 0', overflow: 'hidden' }}>
                <div className="trust-ticker" style={{ display: 'flex', gap: '3rem', whiteSpace: 'nowrap', width: 'max-content' }}>
                    {[...ticker].reverse().map((t, i) => (
                        <span key={i} style={{ color: i % 3 === 0 ? 'rgba(104,150,200,0.55)' : i % 3 === 1 ? 'rgba(167,139,250,0.45)' : 'rgba(255,255,255,0.15)', fontSize: '0.82rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase' }}>{t}</span>
                    ))}
                </div>
            </div>
        </section>
...
function ContactMethodCard({ method, index }) {
    ...
    return (
        <motion.div
            ref={ref}
            whileHover={{ y: -6, borderColor: method.accentColor + '44', boxShadow: `0 18px 50px ${method.glowColor}` }}
            transition={{ duration: 0.28, ease: 'easeOut' }}
            style={{ background: method.gradient, border: '1.5px solid rgba(167,139,250,0.18)', borderRadius: '20px', padding: '1.75rem', display: 'flex', alignItems: 'flex-start', gap: '1.1rem', willChange: 'transform', cursor: 'default', boxShadow: '0 4px 20px rgba(124,92,191,0.05)' }}
        >
...
function SuccessScreen({ name, onReset }) {
    return (
        <motion.div
            ...
            style={{ textAlign: 'center', padding: '4rem 2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.25rem' }}
        >
            <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                transition={{ delay: 0.15, duration: 0.5, ease: 'backOut' }}
                style={{ width: '80px', height: '80px', borderRadius: '50%', background: 'linear-gradient(135deg, rgba(16,185,129,0.2), rgba(52,211,153,0.08))', border: '2px solid rgba(16,185,129,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2.2rem', boxShadow: '0 0 30px rgba(16,185,129,0.15)' }}
            >✦</motion.div>
            <div>
            ...
            <motion.button
                whileHover={{ scale: 1.04, background: 'rgba(167,139,250,0.12)' }}
                whileTap={{ scale: 0.97 }}
                onClick={onReset}
                style={{ marginTop: '0.5rem', padding: '0.65rem 1.75rem', background: 'rgba(167,139,250,0.08)', border: '1.5px solid rgba(167,139,250,0.28)', borderRadius: '50px', color: '#7c5cbf', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', letterSpacing: '0.04em', transition: 'background 0.2s' }}
                ...
                {step === 2 && (
                    <motion.form key="step2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.3, ease: 'easeOut' }} onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.25rem' }}>
                            ...
                            <motion.button
                                whileHover={{ scale: !submitting ? 1.02 : 1, boxShadow: !submitting ? '0 8px 28px rgba(124,92,191,0.35)' : 'none' }}
                                whileTap={{ scale: !submitting ? 0.97 : 1 }}
                                type="submit"
                                disabled={submitting || !form.consultationType}
                                style={{ flex: 1, padding: '0.9rem', background: submitting || !form.consultationType ? 'rgba(167,139,250,0.18)' : 'linear-gradient(135deg, #7c5cbf, #a78bfa)', border: 'none', borderRadius: '12px', color: submitting || !form.consultationType ? '#7a94b0' : '#fff', fontSize: '0.93rem', fontWeight: 700, cursor: submitting || !form.consultationType ? 'not-allowed' : 'pointer', letterSpacing: '0.04em', boxShadow: !submitting && form.consultationType ? '0 4px 20px rgba(124,92,191,0.25)' : 'none', transition: 'all 0.2s ease' }}
                            >
                                {submitting
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            gsap.timeline({ defaults: { ease: 'power3.out' } })
                .fromTo(titleRef.current, { opacity: 0, y: 55 }, { opacity: 1, y: 0, duration: 1, delay: 0.2 })
        ...
        <div style={{ background: '#faf9f7', minHeight: '100vh', color: '#1e3a5f', fontFamily: "'Inter', sans-serif", overflowX: 'hidden' }}>
            ...
            <section ref={heroRef} style={{ position: 'relative', minHeight: '58vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', background: 'linear-gradient(160deg, #fdf8ff 0%, #f5f0ff 40%, #fff8f5 100%)' }}>
                ...
                <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: '9rem 1.5rem 4rem', maxWidth: '720px', margin: '0 auto' }}>
                    ...
                    <p ref={subtitleRef} style={{ fontSize: 'clamp(0.95rem, 2vw, 1.1rem)', lineHeight: 1.85, color: '#4a6080', maxWidth: '500px', margin: '0 auto', fontWeight: 400 }}>
                        Whether you have a specific situation in mind or simply want to understand what's going on — I'm here to listen and help you figure out the right next step.
                    </p>
                </div>
            </section>
