...
function ServiceGridCard({ service, index, onSelect, isActive }) {
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            gsap.fromTo(el,
                { opacity: 0, y: 55 },
                { delay: index * 0.09, scrollTrigger: { trigger: el, start: 'top 89%' }, opacity: 1, y: 0, duration: 0.7, ease: 'power3.out' }
            );
        });
        return () => ctx.revert();
    }, [index]);
    ...
    return (
        <motion.div ref={ref} onClick={() => onSelect(service)} whileHover={{ y: -10, borderColor: 'rgba(167,139,250,0.45)', boxShadow: `0 20px 56px ${service.glowColor}`, transition: { duration: 0.3, ease: 'easeOut' } }} style={{
            ...
            boxShadow: isActive ? `0 8px 40px ${service.glowColor}, 0 0 0 1.5px ${service.accentColor}22` : '0 4px 24px rgba(124,92,191,0.06)',
...
function ServiceDetailPanel({ service, onClose }) {
    ...
    return (
        <motion.div ref={panelRef} key={service.id} variants={panelVariants} initial="hidden" animate="visible" exit="exit" style={{
            ...
            <motion.button whileHover={{ scale: 1.1, background: 'rgba(167,139,250,0.15)' }} whileTap={{ scale: 0.95 }} onClick={onClose} style={{ position: 'absolute', top: '1.5rem', right: '1.5rem', width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(167,139,250,0.08)', border: '1.5px solid rgba(167,139,250,0.22)', color: '#7c5cbf', fontSize: '1.1rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2 }}>x</motion.button>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '2rem' }}>
                ...
                <motion.div variants={panelChildVariants} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    ...
                    <div style={{ padding: '1.75rem', background: 'rgba(255,255,255,0.75)', border: '1.5px solid rgba(167,139,250,0.18)', borderRadius: '18px', backdropFilter: 'blur(8px)' }}>
                        <div style={{ color: '#2d2438', fontWeight: 700, fontSize: '1rem', marginBottom: '0.4rem' }}>Interested in this service?</div>
                        <div style={{ color: '#9585b0', fontSize: '0.83rem', lineHeight: 1.7, marginBottom: '1.25rem' }}>Reach out on WhatsApp — I'll understand your requirement and suggest the best approach for you.</div>
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            gsap.fromTo('.process-step',
                { opacity: 0, x: -40 },
                { opacity: 1, x: 0, duration: 0.7, stagger: 0.15, ease: 'power3.out', scrollTrigger: { trigger: el, start: 'top 85%' } }
            );
        }, el);
        return () => ctx.revert();
    }, []);
            ...
            <section ref={heroRef} style={{ position: 'relative', minHeight: '90vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', background: 'linear-gradient(160deg, #fdf8ff 0%, #f5f0ff 40%, #fff8f5 100%)' }}>
                ...
                <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: '9rem 1.5rem 4rem', maxWidth: '760px', margin: '0 auto' }}>
                    ...
                    <h1 ref={titleRef} style={{ fontSize: 'clamp(2.4rem, 6vw, 4.5rem)', fontWeight: 800, lineHeight: 1.1, letterSpacing: '-0.02em', fontFamily: "'Cormorant Garamond', serif", marginBottom: '1.25rem' }}>
                        <span style={{ background: 'linear-gradient(135deg, #3d2b6b 30%, #7c5cbf 70%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Structured Guidance.</span>
                        <br />
                        <span style={{ background: 'linear-gradient(135deg, #a78bfa, #c4b5fd, #f0abfc)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Real-World Results.</span>
                    </h1>
                    <p ref={subtitleRef} style={{ fontSize: 'clamp(0.95rem, 2vw, 1.12rem)', lineHeight: 1.85, color: '#6b5c8a', maxWidth: '540px', margin: '0 auto', fontWeight: 400 }}>Numerology, Vastu, and Jyotish — each one a precise, logical lens designed to help you solve real challenges and make aligned decisions with confidence.
                    </p>
                </div>
            </section>
            ...
            <section ref={processRef} style={{ padding: '7rem 1.5rem', background: 'linear-gradient(180deg, transparent, rgba(245,240,255,0.7), transparent)', borderTop: '1px solid rgba(167,139,250,0.12)' }}>
                <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
                    <div className="reveal-up" style={{ textAlign: 'center', marginBottom: '4.5rem' }}>
                        ...
                        <p style={{ color: '#9585b0', fontSize: '0.92rem', maxWidth: '400px', margin: '0 auto', lineHeight: 1.75 }}>Here is exactly what happens from the moment you reach out.</p>
                    </div>
                    ...
                    <div className="reveal-up" style={{ marginTop: '2.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem' }}>
                        {[
                            ...
                            <div key={e.text} style={{ display: 'flex', alignItems: 'center', gap: '0.65rem', padding: '1rem 1.25rem', background: '#ffffff', border: '1.5px solid rgba(167,139,250,0.15)', borderRadius: '14px', boxShadow: '0 2px 12px rgba(124,92,191,0.04)' }}>
                                ...
                                <span style={{ color: '#6b5c8a', fontSize: '0.82rem', fontWeight: 500, lineHeight: 1.5 }}>{e.text}</span>
                            </div>
                        ))}
                    </div>
            ...
            <section style={{ padding: '2rem 1.5rem 7rem' }}>
                <div style={{ maxWidth: '780px', margin: '0 auto' }}>
                    <div className="reveal-up" style={{ textAlign: 'center', marginBottom: '3rem' }}>
                        ...
                        <h2 style={{ fontSize: 'clamp(1.5rem, 3.5vw, 2.5rem)', fontWeight: 800, fontFamily: "'Cormorant Garamond', serif", background: 'linear-gradient(135deg, #3d2b6b, #7c5cbf)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Before You Reach Out</h2>
                    </div>
                    <FAQAccordion />
                </div>
            </section>
            <section style={{ padding: '0 1.5rem 8rem' }}>
                <div className="reveal-up" style={{ maxWidth: '820px', margin: '0 auto' }}>
                    <motion.div whileHover={{ boxShadow: '0 30px 80px rgba(124,92,191,0.15)' }} transition={{ duration: 0.3 }} style={{ background: 'linear-gradient(135deg, rgba(245,240,255,0.95), rgba(255,248,255,0.98))', border: '1.5px solid rgba(167,139,250,0.25)', borderRadius: '28px', padding: 'clamp(2.5rem, 5vw, 4rem)', textAlign: 'center', position: 'relative', overflow: 'hidden', boxShadow: '0 20px 60px rgba(124,92,191,0.07)' }}>
                        ...
                        <p style={{ color: '#6b5c8a', fontSize: '0.97rem', lineHeight: 1.8, maxWidth: '440px', margin: '0 auto 2.25rem' }}>Just start a conversation on WhatsApp. Share your concern or area of focus and I'll suggest the right consultation for you — no obligation, no pressure.</p>
